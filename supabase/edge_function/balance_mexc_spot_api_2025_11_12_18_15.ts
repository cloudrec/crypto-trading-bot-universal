...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`🔍 BALANCE MEXC SPOT: ${action} for ${exchange || 'all exchanges'}`);
    ...
    console.error('❌ BALANCE MEXC SPOT Error:', error.message);
    return new Response(
...
async function checkSingleExchangeBalance(supabaseClient: any, user_id: string, exchange: string) {
  console.log(`🔍 BALANCE MEXC SPOT: Checking ${exchange}`);
  ...
  try {
    ...
    switch (exchange) {
      ...
      case 'bybit':
        ...
        break;
      case 'mexc':
        balance = await getMexcBalanceSpot(apiKeys);
        break;
      case 'gate':
...
// MEXC Balance - SPOT API ВМЕСТО FUTURES
async function getMexcBalanceSpot(apiKeys: any) {
  console.log('🟠 MEXC SPOT API: Getting balance using SPOT endpoint instead of futures');
  
  try {
    const secretKey = apiKeys.secret || apiKeys.api_secret;
    const timestamp = Date.now();
    
    // ИСПРАВЛЕНО: Используем SPOT API endpoint вместо futures
    // Spot API: /api/v3/account
    const queryString = `timestamp=${timestamp}`;
    
    console.log(`MEXC SPOT API details:`, {
      api_key: apiKeys.api_key.substring(0, 8) + '...',
      timestamp,
      queryString,
      endpoint: 'SPOT /api/v3/account',
      secret_field: apiKeys.secret ? 'secret' : 'api_secret'
    });
    
    // SPOT API подпись: просто queryString
    const signature = await createMEXCSignatureSpot(queryString, secretKey);
    
    // ИСПРАВЛЕНО: Используем SPOT API endpoint
    const response = await fetch(`https://api.mexc.com/api/v3/account?${queryString}&signature=${signature}`, {
      method: 'GET',
      headers: {
        'X-MEXC-APIKEY': apiKeys.api_key,
    ...
    if (!response.ok) {
      ...
      console.error(`MEXC SPOT API error response:`, errorText);
      throw new Error(`MEXC SPOT API error: HTTP ${response.status} - ${errorText}`);
    }

    const data = await response.json();
    console.log('MEXC SPOT API response:', data);
    
    // SPOT API возвращает другую структуру
    if (data.balances) {
      const usdtBalance = data.balances.find((balance: any) => balance.asset === 'USDT');
      const balance = usdtBalance ? parseFloat(usdtBalance.free || usdtBalance.locked || '0') : 0;
      
      console.log('🟠 MEXC SPOT: Balance extracted:', balance);
      return balance;
    } else {
      throw new Error(`MEXC SPOT API error: ${data.msg || data.message || 'Unknown error'}`);
    }
    
  } catch (error) {
    console.error('❌ MEXC SPOT balance error:', error.message);
    throw error;
  }
}

// MEXC Signature SPOT - ПРОСТАЯ ПОДПИСЬ ДЛЯ SPOT API
async function createMEXCSignatureSpot(queryString: string, secret: string): Promise<string> {
