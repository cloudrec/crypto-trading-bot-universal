...
serve(async (req) => {
  ...
  try {
    ...
    console.log('🟠 Gate.io Test Order Request:', {
      exchange, symbol, side, leverage, amount, totalAmount, stopLoss, takeProfit, delayMs
    });
    ...
    const mockOrder = {
      orderId: `gate_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`,
    ...
    console.log('✅ Gate.io Mock Order Created:', mockOrder);

    return new Response(
      JSON.stringify({
        success: true,
        order: mockOrder,
        message: `🟠 Gate.io тестовый ордер размещен: ${mockOrder.orderId}`
      }),
      { headers: { ...corsHeaders, 'Content-Type': 'application/json' }, status: 200 }
    );

  } catch (error) {
    console.error('❌ Gate.io Test Order Error:', error);
    return new Response(
      JSON.stringify({
        success: false,
        error: error.message,
        message: '❌ Ошибка размещения тестового ордера на Gate.io'
      }),
