...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`🔍 UNIVERSAL BALANCE: ${action} for ${exchange || 'all exchanges'}`);

    if (action === 'check_all_balances') {
      return await checkAllExchangeBalances(supabaseClient, user_id);
    } else if (action === 'check_balance' && exchange) {
    ...
    console.error('❌ UNIVERSAL BALANCE Error:', error.message);
    return new Response(
...
// Проверка баланса всех бирж
async function checkAllExchangeBalances(supabaseClient: any, user_id: string) {
  console.log('🔍 UNIVERSAL BALANCE: Checking all exchanges');
  
  const exchanges = ['binance', 'bybit', 'mexc', 'gate', 'okx', 'kucoin'];
  const results = [];

  for (const exchange of exchanges) {
    try {
      const result = await checkSingleExchangeBalance(supabaseClient, user_id, exchange);
      const data = await result.json();
      results.push({
        exchange,
        name: getExchangeName(exchange),
        ...data
      });
    } catch (error) {
      results.push({
  ...
  return new Response(
    JSON.stringify({ success: true, balances: results }),
    { headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
  );
}
...
async function checkSingleExchangeBalance(supabaseClient: any, user_id: string, exchange: string) {
  console.log(`🔍 UNIVERSAL BALANCE: Checking ${exchange}`);
...
async function getBybitBalance(apiKeys: any) {
  ...
  const signature = await createBybitSignature(timestamp, apiKeys.key, recvWindow, queryString, apiKeys.secret);
  
  const response = await fetch(`https://api.bybit.com/v5/account/wallet-balance?${queryString}`, {
    method: 'GET',
    headers: {
      'X-BAPI-API-KEY': apiKeys.key,
...
async function getMexcBalance(apiKeys: any) {
  ...
  if (!response.ok) {
    throw new Error(`MEXC API error: HTTP ${response.status}`);
  }
...
async function getKucoinBalance(apiKeys: any) {
  ...
  const signature = await createHmacSignatureBase64(message, apiKeys.secret);
  const passphrase = await createHmacSignatureBase64(apiKeys.passphrase || '', apiKeys.secret);
  
  const response = await fetch(`https://api-futures.kucoin.com${endpoint}`, {
    method: 'GET',
    headers: {
      'KC-API-KEY': apiKeys.key,
