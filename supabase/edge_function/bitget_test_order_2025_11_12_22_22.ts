...
serve(async (req) => {
  ...
  try {
    ...
    console.log('🟢 Bitget Test Order Request:', {
    ...
    const mockOrder = {
      orderId: `bitget_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`,
    ...
    console.log('✅ Bitget Mock Order Created:', mockOrder);

    return new Response(
      JSON.stringify({
        success: true,
        order: mockOrder,
        message: `🟢 Bitget тестовый ордер размещен: ${mockOrder.orderId}`
      }),
      { headers: { ...corsHeaders, 'Content-Type': 'application/json' }, status: 200 }
    );

  } catch (error) {
    console.error('❌ Bitget Test Order Error:', error);
    return new Response(
      JSON.stringify({
        success: false,
        error: error.message,
        message: '❌ Ошибка размещения тестового ордера на Bitget'
      }),
