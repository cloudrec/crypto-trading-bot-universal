...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`🔍 BALANCE MEXC FIXED: ${action} for ${exchange || 'all exchanges'}`);
    ...
    console.error('❌ BALANCE MEXC FIXED Error:', error.message);
    return new Response(
...
async function checkSingleExchangeBalance(supabaseClient: any, user_id: string, exchange: string) {
  console.log(`🔍 BALANCE MEXC FIXED: Checking ${exchange}`);
...
// MEXC Balance - ПОЛНОСТЬЮ ИСПРАВЛЕННЫЙ
async function getMexcBalance(apiKeys: any) {
  const timestamp = Date.now();
  
  // ИСПРАВЛЕНО: Правильный формат подписи для MEXC
  const queryString = `timestamp=${timestamp}`;
  const signature = await createHmacSignature(queryString, apiKeys.secret);
  
  console.log(`MEXC request details:`, {
    timestamp,
    queryString,
  ...
  // ИСПРАВЛЕНО: Используем правильный endpoint для MEXC futures
  ...
  // ИСПРАВЛЕНО: Правильные поля для MEXC futures
  if (data.data && Array.isArray(data.data)) {
    const usdtAsset = data.data.find((asset: any) => asset.currency === 'USDT' || asset.asset === 'USDT');
    if (usdtAsset) {
      return parseFloat(usdtAsset.availableBalance || usdtAsset.equity || usdtAsset.balance || 0);
    }
  }
