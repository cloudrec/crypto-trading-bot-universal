...
serve(async (req) => {
  ...
  try {
    ...
    console.log('🔴 HTX (Huobi) Test Order Request:', {
    ...
    const mockOrder = {
      orderId: `htx_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`,
    ...
    console.log('✅ HTX Mock Order Created:', mockOrder);

    return new Response(
      JSON.stringify({
        success: true,
        order: mockOrder,
        message: `🔴 HTX (Huobi) тестовый ордер размещен: ${mockOrder.orderId}`
      }),
      { headers: { ...corsHeaders, 'Content-Type': 'application/json' }, status: 200 }
    );

  } catch (error) {
    console.error('❌ HTX Test Order Error:', error);
    return new Response(
      JSON.stringify({
        success: false,
        error: error.message,
        message: '❌ Ошибка размещения тестового ордера на HTX'
      }),
