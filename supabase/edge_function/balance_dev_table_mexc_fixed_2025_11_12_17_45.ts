...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`🔍 BALANCE DEV TABLE: ${action} for ${exchange || 'all exchanges'}`);
    ...
    console.error('❌ BALANCE DEV TABLE Error:', error.message);
    return new Response(
...
async function checkSingleExchangeBalance(supabaseClient: any, user_id: string, exchange: string) {
  console.log(`🔍 BALANCE DEV TABLE: Checking ${exchange}`);

  // ИСПРАВЛЕНО: Используем правильную таблицу api_keys_dev
  ...
  if (keysError || !apiKeys) {
    console.log(`❌ API ключи для ${exchange} не найдены в api_keys_dev:`, keysError);
    return new Response(
      JSON.stringify({ 
        success: false, 
  ...
  console.log(`🔍 Найдены ключи для ${exchange} в api_keys_dev:`, {
    api_key: apiKeys.api_key ? `${apiKeys.api_key.substring(0, 8)}...` : 'пусто',
    api_secret: apiKeys.api_secret ? `${apiKeys.api_secret.substring(0, 8)}...` : 'пусто',
    secret: apiKeys.secret ? `${apiKeys.secret.substring(0, 8)}...` : 'пусто',
    passphrase: apiKeys.passphrase ? '***' : 'пусто'
  });

  // ИСПРАВЛЕНО: Проверяем оба поля secret и api_secret
  const secretKey = apiKeys.api_secret || apiKeys.secret;
  ...
  try {
    ...
    switch (exchange) {
      ...
      case 'bybit':
        ...
        break;
      case 'mexc':
        balance = await getMexcBalanceOriginal(apiKeys);
        break;
      case 'gate':
...
// MEXC Balance - ОРИГИНАЛЬНАЯ РАБОЧАЯ ВЕРСИЯ
async function getMexcBalanceOriginal(apiKeys: any) {
  console.log('🟠 MEXC ORIGINAL: Getting balance from api_keys_dev');
  
  try {
    const timestamp = Date.now();
    const queryString = `timestamp=${timestamp}`;
    
    // ИСПРАВЛЕНО: Используем api_secret если есть, иначе secret
    const secretKey = apiKeys.api_secret || apiKeys.secret;
    const signature = await createMEXCSignatureOriginal(queryString, secretKey);
    
    console.log(`MEXC ORIGINAL request details:`, {
      timestamp,
      queryString,
      signature: signature.substring(0, 10) + '...',
      api_key: apiKeys.api_key.substring(0, 8) + '...',
      secret_field: apiKeys.api_secret ? 'api_secret' : 'secret'
    });
    ...
    if (!response.ok) {
      ...
      console.error(`MEXC ORIGINAL API error response:`, errorText);
      throw new Error(`MEXC API error: HTTP ${response.status} - ${errorText}`);
    }

    const data = await response.json();
    console.log('MEXC ORIGINAL API response:', data);
    ...
    console.log('🟠 MEXC ORIGINAL: Balance extracted:', balance);
    
    return balance;
    
  } catch (error) {
    console.error('❌ MEXC ORIGINAL balance error:', error.message);
    throw error;
  }
}

// MEXC Signature ORIGINAL - ТОЧНО РАБОЧАЯ ВЕРСИЯ
async function createMEXCSignatureOriginal(queryString: string, secret: string): Promise<string> {
...
async function getGateBalance(apiKeys: any) {
  ...
  if (!response.ok) {
    const errorText = await response.text();
    console.error(`Gate.io API error response:`, errorText);
    throw new Error(`Gate.io API error: HTTP ${response.status} - ${errorText}`);
  }

  const data = await response.json();
  console.log('Gate.io API response:', data);
  
  // Правильное поле для баланса
