...
serve(async (req) => {
  ...
  try {
    ...
    const positionSize = (parseFloat(totalAmount || amount) / 50000).toFixed(6); // Assuming BTC price ~50k
    ...
    return new Response(
      JSON.stringify({
        success: true,
        order: mockOrder,
        message: `🎯 Bybit тестовый ордер размещен: ${mockOrder.orderId}`
      }),
      {
    ...
    return new Response(
      JSON.stringify({
        success: false,
        error: error.message,
        message: '❌ Ошибка размещения тестового ордера на Bybit'
      }),
      {
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        status: 500,
