...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`🔍 BALANCE MEXC OFFICIAL: ${action} for ${exchange || 'all exchanges'}`);
    ...
    console.error('❌ BALANCE MEXC OFFICIAL Error:', error.message);
    return new Response(
...
async function checkSingleExchangeBalance(supabaseClient: any, user_id: string, exchange: string) {
  console.log(`🔍 BALANCE MEXC OFFICIAL: Checking ${exchange}`);
  ...
  try {
    ...
    switch (exchange) {
      ...
      case 'bybit':
        ...
        break;
      case 'mexc':
        balance = await getMexcBalanceOfficial(apiKeys);
        break;
      case 'gate':
...
// MEXC Balance - ПРАВИЛЬНАЯ ПОДПИСЬ ПО ОФИЦИАЛЬНОЙ ДОКУМЕНТАЦИИ
async function getMexcBalanceOfficial(apiKeys: any) {
  console.log('🟠 MEXC OFFICIAL DOCS: Getting balance with correct signature');
  
  try {
    const secretKey = apiKeys.secret || apiKeys.api_secret;
    const timestamp = Date.now().toString();
    
    // ИСПРАВЛЕНО: Правильная подпись по официальной документации MEXC
    // Signature target string: accessKey + timestamp + parameterString
    const parameterString = `timestamp=${timestamp}`;
    const signatureTarget = apiKeys.api_key + timestamp + parameterString;
    
    console.log(`MEXC OFFICIAL signature details:`, {
      api_key: apiKeys.api_key.substring(0, 8) + '...',
      timestamp,
      parameterString,
      signatureTarget: signatureTarget.substring(0, 20) + '...',
      secret_field: apiKeys.secret ? 'secret' : 'api_secret'
    });
    
    const signature = await createMEXCSignatureOfficial(signatureTarget, secretKey);
    
    // ИСПРАВЛЕНО: Правильные заголовки по документации
    const response = await fetch(`https://contract.mexc.com/api/v1/private/account/assets?${parameterString}&signature=${signature}`, {
      method: 'GET',
      headers: {
    ...
    if (!response.ok) {
      ...
      console.error(`MEXC OFFICIAL API error response:`, errorText);
      throw new Error(`MEXC API error: HTTP ${response.status} - ${errorText}`);
    }

    const data = await response.json();
    console.log('MEXC OFFICIAL API response:', data);
    
    if (!data.success) {
      throw new Error(`MEXC API error: ${data.message || data.errorMsg || 'Unknown error'}`);
    }
    ...
    console.log('🟠 MEXC OFFICIAL: Balance extracted:', balance);
    
    return balance;
    
  } catch (error) {
    console.error('❌ MEXC OFFICIAL balance error:', error.message);
    throw error;
  }
}

// MEXC Signature OFFICIAL - ПО ОФИЦИАЛЬНОЙ ДОКУМЕНТАЦИИ
async function createMEXCSignatureOfficial(signatureTarget: string, secret: string): Promise<string> {
  const encoder = new TextEncoder();
  const keyData = encoder.encode(secret);
  const messageData = encoder.encode(signatureTarget);
