...
serve(async (req) => {
  ...
  try {
    ...
    console.log('⚫ OKX Test Order Request:', {
    ...
    console.log('✅ OKX Mock Order Created:', mockOrder);

    return new Response(
      JSON.stringify({
        success: true,
        order: mockOrder,
        message: `⚫ OKX тестовый ордер размещен: ${mockOrder.orderId}`
      }),
      { headers: { ...corsHeaders, 'Content-Type': 'application/json' }, status: 200 }
    );

  } catch (error) {
    console.error('❌ OKX Test Order Error:', error);
    return new Response(
      JSON.stringify({
        success: false,
        error: error.message,
        message: '❌ Ошибка размещения тестового ордера на OKX'
      }),
