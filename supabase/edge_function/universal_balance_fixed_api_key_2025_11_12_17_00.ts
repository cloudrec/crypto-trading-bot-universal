...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`🔍 UNIVERSAL BALANCE FIXED: ${action} for ${exchange || 'all exchanges'}`);
    ...
    console.error('❌ UNIVERSAL BALANCE FIXED Error:', error.message);
    return new Response(
...
async function checkAllExchangeBalances(supabaseClient: any, user_id: string) {
  console.log('🔍 UNIVERSAL BALANCE FIXED: Checking all exchanges');
...
async function checkSingleExchangeBalance(supabaseClient: any, user_id: string, exchange: string) {
  console.log(`🔍 UNIVERSAL BALANCE FIXED: Checking ${exchange}`);
