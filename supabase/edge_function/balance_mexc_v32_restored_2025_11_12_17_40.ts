...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`🔍 BALANCE RESTORED V32: ${action} for ${exchange || 'all exchanges'}`);
    ...
    console.error('❌ BALANCE RESTORED V32 Error:', error.message);
    return new Response(
...
async function checkSingleExchangeBalance(supabaseClient: any, user_id: string, exchange: string) {
  console.log(`🔍 BALANCE RESTORED V32: Checking ${exchange}`);
  ...
  try {
    ...
    switch (exchange) {
      ...
      case 'bybit':
        ...
        break;
      case 'mexc':
        balance = await getMexcBalanceV32(apiKeys);
        break;
      case 'gate':
...
// MEXC Balance - ВОССТАНОВЛЕННАЯ РАБОЧАЯ ВЕРСИЯ V32
async function getMexcBalanceV32(apiKeys: any) {
  ...
  try {
    ...
    // ВАЖНО: Используем рабочую логику из v32
    const signature = await createMEXCSignatureV32(queryString, apiKeys.secret);
    
    console.log(`MEXC V32 request details:`, {
      timestamp,
      queryString,
    ...
    if (!response.ok) {
      ...
      console.error(`MEXC V32 API error response:`, errorText);
      throw new Error(`MEXC API error: HTTP ${response.status} - ${errorText}`);
    }

    const data = await response.json();
    console.log('MEXC V32 API response:', data);
...
// MEXC Signature V32 - РАБОЧАЯ ВЕРСИЯ
async function createMEXCSignatureV32(queryString: string, secret: string): Promise<string> {
